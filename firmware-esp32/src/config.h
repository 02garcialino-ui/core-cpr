...
// ---- Bus I2C real (ADS1115 + MAX30102 + VL53L0X, T9.4) ----
// Los 3 sensores reales comparten el mismo bus de 2 cables (SDA/SCL), cada
// uno con su propia direccion (no gastan un GPIO cada uno). La ESP32 usa
// por defecto GPIO 21 = SDA y GPIO 22 = SCL (Wire.begin() sin argumentos).
// OJO: estos dos pines quedan RESERVADOS solo para I2C en hardware real,
// no se reusan para otra cosa (ver nota en PIN_BOTON_REINTENTAR mas abajo,
// que todavia usa el 21 y esta pendiente de reubicar).
...
#define UMBRAL_PPG_MV    1650    // milivolts (SOLO modo simulado)

// El MAX30102 real no entrega milivolts como el potenciometro simulado:
// entrega una cuenta cruda de luz infrarroja reflejada (mas alta = dedo
// presente, bloqueando el sensor de luz ambiente). Por eso hace falta un
// umbral aparte para el modo real. OJO: PROVISIONAL -- es un valor de
// arranque tipico de los ejemplos de la libreria SparkFun, se recalibra
// mirando lo que el sensor real entrega con y sin dedo (T9.4).
#define UMBRAL_PPG_IR    50000
...
// Pulsador normal (no NC como el de emergencia). GPIO 21: unico pin libre
// con pull-up interno disponible mientras solo existia la simulacion.
// PENDIENTE (no urgente): GPIO 21 ahora es SDA del bus I2C real (ver nota
// arriba) -- choca con este boton en cuanto se corra el firmware completo
// en hardware real con MODO_SIMULACION false. Se resuelve reasignando este
// pin cuando llegue el pulsador fisico (no antes, para no complicar sin
// necesidad): candidatos son GPIO 36/39 (libres, pero sin pull-up interno,
// necesitan una resistencia externa a 3.3V) -- confirmar el rotulo exacto
// en la placa fisica antes de elegir, hay clones que los marcan "SVP"/"SVN"
// en vez del numero.
...
// real) se reasigna a GPIO 32 (se libera solo al dejar de usarse como
// potenciometro de profundidad simulada, PIN_PROFUNDIDAD_SIM). Antes se
// habia pensado GPIO 22, pero ese pin quedo reservado para el bus I2C
// real (SCL) -- ver la nota junto a PIN_ECG_SIM.
#define PIN_BUZZER  0
